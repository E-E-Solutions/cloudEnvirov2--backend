...
const loginController = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      try {
        resolvedVendorId = await decryptVendorId(
          vendorId,
    ...
    if (!currentUser) {
      ...
      const [existedVendorId] = await Reseller.vendorIdExists(
        email,
        resolvedVendorId
      );
      ...
      if (
        userRole === "reseller" &&
        existedVendorId[0].vendor_id === resolvedVendorId
      ) {
      ...
      if (!currentUser) {
        const [resellerUserExist] = await Reseller.findResellersUserByEmailId(
          email
        );
        if (!resellerUserExist[0]) {
          ...
          if (userResult[0]) {
            ...
            console.log({
              ...
              resolvedVendorId,
              productsList: currentUser.products_list,
            });
            const firmName = currentUser.firm_name || "";
            const contactNo = currentUser.contact || "";
            const address = currentUser.address || "";
            const productsList = currentUser.products_list
              ? JSON.parse(currentUser.products_list)
              : [];
            const newReseller = await Users.createResellerUser(
              email,
              password,
              firmName,
              contactNo,
        ...
        if (resellerUserExist[0]) {
          const [existedVendorId] = await Reseller.vendorUserIdExists(
            email,
            resolvedVendorId
          );
          ...
          if (
            (existedVendorId &&
              existedVendorId[0].vendor_id === resolvedVendorId) ||
            existedVendorId[0].vendor_id === null
          ) {
            const [matchVendorId] = await Reseller.checkVendorId(
              resolvedVendorId
            );
            ...
            if (matchVendorId.length <= 0) {
              ...
              if (!isAllowedAccess.accessStatus) {
                return res.status(404).json({
                  success: false,
                  message:
                    "Your service has ended. Please contact the account administrator for assistance.",
                });
              }
              const [resellerUserResult] =
                await Reseller.findResellersUserByEmailId(email);
              ...
              if (isReseller) {
                ...
                try {
                  ...
                  const updatedId = await Reseller.addVendorId(
                    resolvedVendorId,
                    email
                  );
                } catch (err) {
                ...
                const [resellerRows] = await Reseller.findReseller(
                  resolvedVendorId
                );
                if (resellerRows.length > 0) {
                  const resellerEmail = resellerRows[0].email;

                  const resellerDevices = await Reseller.fetchResellerDevices(
                    resellerEmail
                  );
                  ...
                  if (productsListRaw !== null) {
                    ...
                    mergedList = Array.from(
                      new Set([...parsedList, ...deviceIds])
                    );
                  } else {
                    mergedList = deviceIds;
                  }

                  updateResult = await Admin.updateResellerDeviceInfo(
                    resellerEmail,
                    mergedList
                  );
...
const registerController = async (req, res) => {
  try {
    ...
    if (resolvedVendorId) {
      const resellerUser = await Reseller.findResellerUser(
        resolvedVendorId,
        email
      );
      if (resellerUser[0][0]) {
      ...
      const newUser = await Users.createResellerUser(
        email,
        password,
        firmName,
        contactNo,
...
const changePasswordController = async (req, res) => {
  try {
    ...
    if (role === "resellerUser") {
      updatePassword = await Reseller.changePassword(
        email,
        oldPassword,
        newPassword
      );
    } else {
      updatePassword = await Users.changePassword(
        email,
        oldPassword,
        newPassword
      );
...
const updateFirmInfoController = async (req, res) => {
  try {
    ...
    let updatedUser;
    if (role === "resellerUser") {
      const [resellerUpdated] = await Reseller.findResellersUserByEmailId(
        email
      );
    ...
    const logActivity = await Users.logUserActivity(
      role,
      email,
      "Update Firm Info",
      "User updated firm info",
      "success",
      "",
      { firmName, firmAddress, contactNo }
    );
