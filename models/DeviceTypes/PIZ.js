...
class PIZ {
        ...
        if (latestRow.length > 0) {
          ...
          if (avgColumns) {
            if (average.includes("min")) {
              ...
              console.log({ average });
              const avgQuery = `SELECT DATE(ts) AS day, DATE_FORMAT(ts, '%Y-%m-%d %H:%i:00') AS time_interval, ${avgColumns} FROM ?? WHERE ts >= NOW() - INTERVAL ? DAY GROUP BY day, FLOOR(MINUTE(ts) / ?), HOUR(ts) ORDER BY day, time_interval;`;
              // Fetch the averages for the day of the latest data point
              const avgResult = await db.query(avgQuery, [
                deviceId,
                days,
              ...
              const avgQuery = `SELECT DATE(ts) AS day,  ${avgColumns} FROM ?? WHERE ts >= NOW() - INTERVAL ? DAY GROUP BY day ORDER BY day;`;
              // Fetch the averages for the day of the latest data point
              const avgResult = await db.query(avgQuery, [deviceId, days]);
              console.log({ avgValue: avgResult[0] });
              resolve({ avgData: avgResult[0] });
            } else if (average.includes("month")) {
              const avgQuery = `SELECT  DATE_FORMAT(ts, '%Y-%m') AS month, ${avgColumns} FROM ?? WHERE ts >= NOW() - INTERVAL ? DAY GROUP BY year(ts), month(ts) ORDER BY month;`;
              // Fetch the averages for the day of the latest data point
              const avgResult = await db.query(avgQuery, [deviceId, days]);
              console.log({ avgValue: avgResult[0] });
              resolve({ avgData: avgResult[0] });
        ...
        if (latestRow.length > 0) {
          ...
          if (avgColumns) {
            if (duration.includes("day")) {
              ...
              const avgQuery = `WITH last_date AS (SELECT MAX(DATE(ts)) AS max_date FROM ??) SELECT DATE_FORMAT(ts, '%Y-%m-%d, %H:00') AS timeStamp, ${avgColumns} FROM ?? WHERE ts BETWEEN (SELECT max_date - INTERVAL ? DAY FROM last_date) AND (SELECT max_date + INTERVAL 1 DAY - INTERVAL 1 SECOND FROM last_date)  GROUP BY timeStamp ORDER BY timeStamp;`;
              // Fetch the averages for the day of the latest data point
              const avgResult = await db.query(avgQuery, [
                deviceId,
                deviceId,
              ...
              duration = duration.split("_")[0];
              const avgQuery = `WITH last_date AS (SELECT MAX(DATE(ts)) AS max_date FROM ??) SELECT DATE_FORMAT(ts, '%Y-%m-%d') AS timeStamp, ${avgColumns} FROM ?? WHERE ts BETWEEN (SELECT max_date - INTERVAL ? MONTH FROM last_date) AND (SELECT max_date + INTERVAL 1 DAY - INTERVAL 1 SECOND FROM last_date) GROUP BY DATE_FORMAT(ts, '%Y-%m-%d') ORDER BY timeStamp;`;
              // Fetch the averages for the day of the latest data point
              const avgResult = await db.query(avgQuery, [
                deviceId,
                deviceId,
        ...
        if (latestRow.length > 0) {
          ...
          if (avgColumns) {
            if (average.includes("hourly")) {
              ...
              const avgQuery = `SELECT DATE_FORMAT(ts, '%Y-%m-%d') AS timeStamp, ${avgColumns} FROM ?? WHERE ts BETWEEN ? AND ? GROUP BY DATE_FORMAT(ts, '%Y-%m-%d') ORDER BY timeStamp;`;
              // Fetch the averages for the day of the latest data point
  ...
  static getDataPoints(deviceId, year) {
    return new Promise((resolve, reject) => {
      try {
        const row = db.query(
          "SELECT DATE(ts) AS date, COUNT(*) AS data_points FROM ?? WHERE YEAR(ts) = ? GROUP BY DATE(ts) ORDER BY date",
          [deviceId, year]
        );
        resolve(row);
      } catch (er) {
  ...
  static getMaxDataPointValue(deviceId, year) {
    return new Promise((resolve, reject) => {
      try {
        const row = db.query(
          "SELECT MAX(daily_data_points) AS max_data_points FROM (SELECT DATE(ts) AS date, COUNT(*) AS daily_data_points FROM ?? WHERE YEAR(ts) = ? GROUP BY DATE(ts)) AS daily_counts",
          [deviceId, year]
        );
        resolve(row);
      } catch (er) {
  ...
  static getDataAvailabilityYears(deviceId) {
    return new Promise((resolve, reject) => {
      try {
        const row = db.query(
          "SELECT DISTINCT YEAR(ts) AS year FROM ?? ORDER BY year",
          [deviceId]
        );
        resolve(row);
      } catch (er) {
